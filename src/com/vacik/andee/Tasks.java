...
//        }
    ...
    public static SortedMap<Date, Set<Task>> calendar(Iterable<Task> tasks, Date start, Date end){
        ...
        for(Task temp : tasks) { // for ( Iterator<Task> iter = tasks.iterator(); iter.hasNext(); iter.next() )
            for(Date time = temp.nextTimeAfter(start); time != null; time = temp.nextTimeAfter(time))
            {
